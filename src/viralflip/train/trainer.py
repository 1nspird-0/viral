...
class ViralFlipTrainer:
    ...
    def __init__(
        ...
        self.min_lr = train_cfg.get("min_lr", 1e-7)
    ...
    def train(
        ...
        for epoch in range(start_epoch, self.epochs):
            ...
            # Compute mean AUPRC
            val_auprc_mean = np.mean([val_metrics[f"auprc_{h}h"] for h in self.model.horizons])
            
            # Update scheduler
            current_lr = self.optimizer.param_groups[0]["lr"]
            if isinstance(self.scheduler, ReduceLROnPlateau):
                self.scheduler.step(val_auprc_mean)
            ...
            self.history["val_auroc"].append(
                np.mean([val_metrics[f"auroc_{h}h"] for h in self.model.horizons])
            )
            ...
            # Save best model
            if val_auprc_mean >= self.early_stopping.best_score:
    ...
    def _train_epoch(self, loader: DataLoader, epoch: int) -> float:
        ...
        for batch_idx, batch in enumerate(pbar):
            ...
            with autocast(enabled=self.use_amp):
                predictions, confidence, virus_logits = self.model(
    ...
    def _validate(self, loader: DataLoader) -> dict:
        ...
        for batch in loader:
            ...
            with autocast(enabled=self.use_amp):
                predictions, _, virus_logits = self.model(features, missing, quality, user_ids)
                predictions = predictions[:, -1, :]
        ...
        for i, horizon in enumerate(self.model.horizons):
            ...
            if label_h.sum() == 0:
                metrics[f"auprc_{horizon}h"] = 0.0
                metrics[f"auroc_{horizon}h"] = 0.5
            else:
