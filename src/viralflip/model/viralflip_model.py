...
5. Virus Classifier (optional): Multi-class classification of virus type
...
class ViralFlipOutput:
    ...
    # Virus type classification (optional, if classifier is enabled)
...
class ViralFlipModel(nn.Module):
    ...
    def forward(
        ...
        Returns:
            Tuple of (risk_probs, confidence_scores, virus_logits).
            risk_probs: (batch, seq_len, n_horizons).
            confidence_scores: (batch, seq_len) or (batch,).
            virus_logits: (batch, seq_len, n_virus_classes) or None if classifier disabled.
        """
    ...
    def predict_single(
        ...
        with torch.no_grad():
            ...
            # Forward
            risk_probs, confidence, virus_logits = self.forward(
                drift_tensors,
                missing_tensors,
                quality_tensors,
            ...
            risk_probs_np = risk_probs[0, -1, :].numpy()
            ...
            if virus_logits is not None:
                ...
                virus_probs_np = virus_probs_tensor.numpy()
                
                # Build virus probability dict
                virus_probs = {}
                for vt in VirusType:
        ...
        confidences = {h: confidence_val for h in self.horizons}
        raw_logits = {
